...
#define OLED_STATDISPLAY_Y 0

void render_stat(void);
...
void render_UI_frame(void) {
	...
	oled_set_cursor(OLED_STATDISPLAY_X, OLED_STATDISPLAY_Y+1);
	oled_write_P(PSTR("RGB       BRT    %"), false);
...
void render_stat(void) {
	// Show current layer
	oled_set_cursor(OLED_STATDISPLAY_X+6, OLED_STATDISPLAY_Y);
    oled_write_char(get_highest_layer(layer_state) + 0x30, false);
	
	// Show current RGB mode (#__)
	oled_set_cursor(OLED_STATDISPLAY_X+4, OLED_STATDISPLAY_Y+1);
	if (rgb_matrix_is_enabled()) {
		oled_write_P(PSTR("#"), false);
	    oled_write_char(rgb_matrix_get_mode()/10 + 0x30, false);
		oled_write_char(rgb_matrix_get_mode()%10 + 0x30, false);
	}
	// Or RGB OFF
	else {
	...
	// Show RGB brightness (___%)
	oled_set_cursor(OLED_STATDISPLAY_X+14, OLED_STATDISPLAY_Y+1);
	if (rgb_matrix_is_enabled()) {
		char rgb_brightness = rgb_matrix_get_val()*100/255;
		if (rgb_brightness >= 100)
			oled_write_char(rgb_brightness/100 + 0x30, false);
		else oled_write_char(0x20, false);
		if (rgb_brightness >= 10)
			oled_write_char(rgb_brightness%100/10 + 0x30, false);
		else oled_write_char(0x20, false);
		oled_write_char(rgb_brightness%10 + 0x30, false);
	}
	else {
...
void keyboard_post_init_user(void) {
	...
	render_logo();
	oled_set_cursor(0,5);
	oled_write_P(PSTR("   Kawii9 ver 2"), false);
	oled_set_cursor(0,7);
	oled_write_P(PSTR("   Firmware v1.1"), false);
	wait_ms(2000);
	oled_write_P(PSTR(" ok"), false);
	//render_UI_frame();
}
