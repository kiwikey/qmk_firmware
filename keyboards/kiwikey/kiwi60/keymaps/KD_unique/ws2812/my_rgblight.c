...
static uint8_t myeffect_start_pos   = 16;
static uint8_t myeffect_end_pos     = RGBLED_NUM;
static uint8_t myeffect_num_leds    = RGBLED_NUM-16;
...
// KiwiKey added START ->
void my_rgblight_setrgb(uint8_t r, uint8_t g, uint8_t b) {
    if (!rgblight_config.enable) {
        return;
    }

    for (uint8_t i = my_effect_start_pos; i < my_effect_end_pos; i++) {
        led[i].r = r;
        led[i].g = g;
        led[i].b = b;
#ifdef RGBW
...
// KiwiKey added START ->
void my_rgblight_effect_rgbtest(animation_status_t *anim) {
    static uint8_t maxval = 0;
    uint8_t        g;
    uint8_t        r;
    uint8_t        b;
