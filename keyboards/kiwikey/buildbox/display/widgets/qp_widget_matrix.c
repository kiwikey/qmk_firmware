...
void widget_matrix_init(void) {
    ...
    for (uint8_t x = 0; x < MATRIX_ROWS; x++) {
        for (uint8_t y = 0; y < MATRIX_COLS; y++) {
			widget_matrix_draw_button(x, y, WIDGET_MATRIX_COLOR_OFF, false);
		}
	}
	// Render keymap
...
void widget_matrix_draw_button(uint8_t x, uint8_t y, uint8_t hue, uint8_t sat, uint8_t val, bool filled) { // x and y are matrix [x,y], not pixel-related
	// Button background
	qp_rect(my_display,
...
void widget_matrix_keymap_render(void) {
	...
	for (uint8_t i = 0; i < MATRIX_ROWS; i++) {
		for (uint8_t j = 0;  j < MATRIX_COLS; j++) {
			uint16_t keycode = dynamic_keymap_get_keycode(0, i, j);
			uint16_t x_offset = WIDGET_MATRIX_POSX + j* (WIDGET_MATRIX_KEY_WIDTH  + WIDGET_MATRIX_KEY_SPACING);
			uint16_t y_offset = WIDGET_MATRIX_POSY + i* (WIDGET_MATRIX_KEY_HEIGHT + WIDGET_MATRIX_KEY_SPACING);
			// Draw keycode string
			switch (keycode) {
...
void widget_matrix_render_kc_layer(uint8_t posx, uint8_t posy, uint16_t keycode) {
	...
	qp_rect(my_display,
			...
			posy + 1,
			posx + 16,
			posy + WIDGET_MATRIX_LABLE_FONT->line_height + 2,
	...
	// Top-left label
	qp_drawtext_recolor(my_display, posx+3, posy+3, WIDGET_MATRIX_LABLE_FONT, buf1, HSV_BLACK, WIDGET_MATRIX_LABEL_LAYER_BG);
	qp_drawtext_recolor_center( my_display,
								posx + WIDGET_MATRIX_KEY_WIDTH/2,
...
void widget_matrix_update(uint8_t col, uint8_t row) {
		...
		if (on)
			widget_matrix_draw_button(row, col, WIDGET_MATRIX_COLOR_ON, false);
		else
			widget_matrix_draw_button(row, col, WIDGET_MATRIX_COLOR_OFF, false);
}
