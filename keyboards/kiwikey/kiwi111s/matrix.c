...
static const pin_t row_pins[MATRIX_ROWS / 2] = {B1, D4, D6, D7, B4, B5};
static const pin_t col_pins[MATRIX_COLS]     = {F5, F6, F7, C7, C6, B6, D5, D3, D2, B3, B2};
...
static void init_pins_MCP23018(void) {
    ...
    mcp23018_errors += !mcp23018_set_config(I2C_ADDR, mcp23018_PORTB, 0b11000000);
}
...
static uint16_t read_cols_MCP23018(void) {
    ...
    //uint16_t state = ((tmp & 0b11111111) << 2) | ((tmp & 0b1100000000000000) >> 14);
	uint16_t state = ((tmp & 0b11111111) << 0) | ((tmp & 0b1100000000000000) >> 6);
